...
void AP_MotorsKestrel::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    // allow mapping of the motors that are vane servos
    add_motor_num(AP_MOTORS_CH_VN_1);
    add_motor_num(AP_MOTORS_CH_VN_2);
...
void AP_MotorsKestrel::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{
    // check for reverse tricopter
    _pitch_reversed = frame_type == MOTOR_FRAME_TYPE_PLUSREV;
...
void AP_MotorsKestrel::set_update_rate(uint16_t speed_hz)
{
 ...
 */
void AP_MotorsKestrel::output_motor_mask(float thrust, uint32_t mask, float rudder_dt)
{
...
float AP_MotorsKestrel::get_pitch_factor_json(uint8_t i)
{
    float ret = 0.0f;
    ...
    case AP_MOTORS_MOT_2:
        ret = 0.5f;
        break;
    case AP_MOTORS_MOT_3: // rear motor
        ret = -1.0f;
        break;
    }
...
// Run arming checks
bool AP_MotorsKestrel::arming_checks(size_t buflen, char *buffer) const
{
